...
public static class Avx512
{
    ...
    static void CopyAligned32<A>(in A src, ref A dest, int count)
    {
        ...
        // Mask off the bottom 6 bits (0-63) and then kill the bottom
        // 2 bits which makes everything 32 bit aligned
        var extra = len & 0b11_1100;              

        // Find the end that we can get to with just Vector512 copies  
        var alignedEnd = len - extra;
        
        // Copy the bulk
        for (var i = 0u; i < alignedEnd; i += 64)
        {
            Vector512.LoadUnsafe(in srcBytes).StoreUnsafe(ref destBytes);
            srcBytes = ref Unsafe.Add(ref srcBytes, 64);
            destBytes = ref Unsafe.Add(ref destBytes, 64);
        }
    ...
    static void CopyAligned64<A>(in A src, ref A dest, int count)
    {
        ...
        // Mask off the bottom 6 bits (0-63) and then mask off the bottom
        // 3 bits which makes everything 64 bit aligned
        var extra = len & 0b11_1000;              
