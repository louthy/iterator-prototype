...
public static class Avx128
{
    ...
    static void CopyAligned32<A>(in A src, ref A dest, int count)
    {
        ...
        // Mask off the bottom 4 bits (0-15) and then kill the bottom
        // 2 bits which makes everything 32 bit aligned
        var extra = len & 0b1100;              

        // Find the end that we can get to with just Vector128 copies  
        var alignedEnd = len - extra;
        
        // Copy the bulk
        for (var i = 0u; i < alignedEnd; i += 16)
        {
