...
{
    extension<A>(ref LE.ArrayWriter<A> writer)
    {
        /// <summary>
        ...
        public (A[] Buffer, long Start, long Count) ToArray()
        {
            ...
            {
                throw new ObjectDisposedException(nameof(LE.ArrayWriter<>));
            }
        }
        ...
        public Root.Arr<A> ToArr()
        {
            if (Interlocked.CompareExchange(ref writer.disposed, 1, 0) == 0)
            {
                ...
                if (rented)
                {
                    var final = new A[count];
                    var bspan = new Span<A>(buffer, start, count);
                    var fspan = new Span<A>(final, 0, count);
                    bspan.CopyTo(fspan);

                    ArrayPool<A>.Shared.Return(buffer);
                    var r = Root.Arr.create(new ReadOnlySpan<A>(final, 0, count));
                    writer.RefDispose();
                    return r;
                }
                else
