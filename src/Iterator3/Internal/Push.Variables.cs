...
static unsafe partial class Push
{
    ...
    public static bool declare<A>(ref StackFrame frame, in A value) =>
        ...
        frame.Add(G.reset<A>(in ix));

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool TEMPdeclare<A>(ref StackFrame frame, in Iter<A> value)
    {
        frame.globals.Add(in value, out var ix);
        frame.Add(G.reset<Iter<A>>(in ix));

        ref var declared = ref frame.globals.DeclaredAt<Iter<A>>(ix);
        declared.SetIdent(111);
        
        //ref var variable = ref frame.globals.At<Iter<A>>(ix);
    ...
    public static bool arg<A>(ref StackFrame frame, in ushort fromEnd) =>
        ...
        frame.Add(G.pullM<A>(in g.Index));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool addConst<A>(ref StackFrame frame, in A value) =>
    ...
    public static bool arg<A>(ref StackFrame frame, in ushort fromEnd) =>
        ...
        // Load a recent global by providing an index from the end of the globals-list
        frame.globals.AtEnd<A>(in fromEnd, out var g) &&
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool addVar<A>(ref StackFrame frame, in A value) =>
        
        // Push the value to the globals-list
        frame.globals.Add(in value, out var ix);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool var<A>(ref StackFrame frame) =>

        var(ref frame, default(A));

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool addVar<A>(ref StackFrame frame) =>

        addVar(ref frame, default(A));*/
}
