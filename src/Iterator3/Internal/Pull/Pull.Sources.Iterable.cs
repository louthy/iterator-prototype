using System.Runtime.CompilerServices;
using IteratorPrototype.Iterator3.Internal;
using IteratorPrototype.Types;
using LanguageExt.Traits;
using StackFrame = IteratorPrototype.Iterator3.Internal.StackFrame;

namespace IteratorPrototype.Iterator3;

static partial class Pull
{
    [MethodImpl(Optimisations.InliningOnly)]
    public static int iterableUnmanaged<T, IS, A>(in StackFrame frame)
        where T : Tr.IterableImmutable<T, IS>
        where IS : unmanaged
        where A : unmanaged
    {
        ref var ta = ref PullManaged.arg1<K<T, A>>(in frame);
        ref var ts = ref PullUnmanaged.arg2<IS>(in frame);
        if(!T.Next(in ta, ref ts, out var x)) return PullState.Void;
        frame.vars.PushUnmanaged(in x, false);
        //Log.value($"return: {x} : {Ty<A>.Pretty}", in frame);
        return PullState.Continue;
    }
    
    [MethodImpl(Optimisations.InliningOnly)]
    public static int iterableManaged<T, IS, A>(in StackFrame frame)
        where T : Tr.IterableImmutable<T, IS>
        where IS : unmanaged
        where A : class
    {
        ref var ta = ref PullManaged.arg1<K<T, A>>(in frame);
        ref var ts = ref PullUnmanaged.arg2<IS>(in frame);
        if(!T.Next(in ta, ref ts, out var x)) return PullState.Void;
        frame.vars.PushManaged(in x, false);
        //Log.value($"return: {x} : {Ty<A>.Pretty}", in frame);
        return PullState.Continue;
    }
        
    [MethodImpl(Optimisations.InliningOnly)]
    public static int iterableStruct<T, IS, A>(in StackFrame frame)
        where T : Tr.IterableImmutable<T, IS>
        where IS : unmanaged
        where A : struct
    {
        ref var ta = ref PullManaged.arg1<K<T, A>>(in frame);
        ref var ts = ref PullUnmanaged.arg2<IS>(in frame);
        if(!T.Next(in ta, ref ts, out var x)) return PullState.Void;
        frame.vars.PushStruct(in x, false);
        //Log.value($"return: {x} : {Ty<A>.Pretty}", in frame);
        return PullState.Continue;
    }
    
    /*
        Unoptimised reference

    [MethodImpl(Optimisations.Default)]
    public static PullState iterable<T, IS, A>(in StackFrame frame)
        where T : Tr.IterableImmutable<T, IS>
        where IS : unmanaged
    {
        // Pop the iterable instance
        arg1<K<T, A>>(in frame, out var ta) &&

        // Read the iterable state global
        arg2<IS>(in frame, out var ts) &&

        // Step the iterable
        T.StepImmutable(in ta, in ts, out var x, out var xs) &&

        // Update the iterable state
        update1(in frame, in xs) &&

        // Return the value
        @return(in frame, in x)
            ? @continue(in frame)
            : empty(in frame);
    }
    */
}
